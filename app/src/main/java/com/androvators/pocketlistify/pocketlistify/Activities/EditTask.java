...
public class EditTask extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(intent.hasExtra("position"))
        {
            ...
            t_name_str = MainActivity.taskItems.get(pos).getName();
            t_detail_str = MainActivity.taskItems.get(pos).getDetails();
            t_date_str = MainActivity.taskItems.get(pos).getDate();
            t_priority_str = MainActivity.taskItems.get(pos).getPriority();
        }
        else {
    ...
    public void saveTaskPressed(View v){
        if(task_name.getText().toString().trim().length() == 0)
            ...
            MainActivity.taskItems.set(pos,taskItem);
            updateOldTask();
